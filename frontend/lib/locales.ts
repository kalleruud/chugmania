...
const no = {
  ...
  admin: {
    ...
    tables: {
      ...
      tournamentStageTracks: 'Turnering baner per runde',
  ...
  tournament: {
    title: 'Turnering',
    tabSession: 'Session',
    tabParticipants: 'Deltakere',
    tabTournament: 'Turnering',
    create: 'Opprett turnering',
    createPageTitle: 'Ny turnering',
    delete: 'Slett turnering',
    previewLoading: 'Oppdaterer forhåndsvisning…',
    previewWorkload: {
      distinctTracks: (n: number) => `Baner i bruk: ${n}`,
      qualificationLaps: 'Kvalifiseringsrunder per spiller: 1',
      tournamentMatchesRange: (min: number, max: number) =>
        `Kamper per spiller (gruppe + sluttspill): ${min}–${max}`,
    },
    progress: (done: number, total: number) =>
      `Matcher: ${done} / ${total} ferdig`,
    progressGroups: (done: number, total: number) =>
      `Gruppespill: ${done} / ${total} ferdig`,
    qualificationTitle: 'Kvalifisering',
    form: {
      name: 'Navn',
      description: 'Beskrivelse',
      qualificationTrack: 'Kvalifiseringsbane',
      groupsCount: 'Antall grupper',
      advancementCount: 'Antall som går videre per gruppe',
      eliminationType: 'Eliminering',
      eliminationSingle: 'Enkel eliminering',
      eliminationDouble: 'Dobbel eliminering',
      stageTracks: 'Baner per runde',
      selectQualificationTrack: 'Velg kvalifiseringsbane',
      selectStageTracks: 'Legg til bane (rekkefølge brukes i rotasjon)',
      noTracksFound: 'Ingen baner funnet',
      submit: 'Opprett turnering',
      readiness: 'Alle matcher må ha bane',
    },
    toast: {
      created: 'Turnering opprettet',
      deleted: 'Turnering slettet',
      deleteLoading: 'Sletter turnering…',
    },
    slot: {
      winnerOfGroup: (g: string) => `Vinner av gruppe ${g}`,
      runnerUpInGroup: (g: string) => `2. plass i gruppe ${g}`,
      placeInGroup: (rank: number, g: string) => `${rank}. plass i gruppe ${g}`,
      winnerOfMatch: (label: string) => `Vinner av ${label}`,
      loserOfMatch: (label: string) => `Taper av ${label}`,
    },
    error: {
      sessionCancelled: 'Session er avlyst.',
      alreadyExists: 'Det finnes allerede en turnering for denne sessionen.',
      tooFewPlayers: 'Minst fire påmeldte (ja) kreves.',
      invalidConfig: 'Ugyldig turneringskonfigurasjon.',
      invalidTrack: 'Ugyldig bane valgt.',
      blankName: 'Turneringen må ha et navn.',
      missingTrack: 'Hver kamp i turneringen må ha en bane.',
      invalidBracketDependency: 'Ugyldig kobling mellom runder i bracket.',
    },
  },
  match: {
    cancel: 'Avlys',
