...
export default class TournamentManager {
  ...
  private static async getTournamentWithDetails(
    ...
    // Fetch all stages with matches and enriched dependency names
    const tournamentStages = await StageManager.getStages(tournamentId)
    ...
    for (const tournamentStage of tournamentStages) {
      if (tournamentStage.stage.level === 'group') {
        for (const match of tournamentStage.matches) {
          if (match.track) {
